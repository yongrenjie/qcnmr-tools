...
**Step 6: Obtain Boltzmann-averaged chemical shifts and coupling constants**
 ...
 - For chemical shifts: navigate to the folder containing the shielding .out files, then run `shieldings.py *.out -a`. This generates a csv file which contains shieldings for all conformers, populations, etc. The chemical shifts are generated by linear scaling. The slopes and intercepts (for 1H and 13C) are hard-coded; they can be changed if desired.
