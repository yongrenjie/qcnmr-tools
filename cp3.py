...
#
# The script requires two text files: cp3_calc.txt (containing the calculated shifts) and cp3_expt.txt (experimental
...
#    here the connectivity or configuration of the molecule, or for experimental shifts, you may want to specify the
#    major or minor diastereomer). The names must not contain any spaces, but whitespace before or after the names is
#    fine.
...
#
# In both cp3_calc.txt and cp3_expt.txt, the atom labels MUST align exactly! Both files can be generated by copy-pasting
# relevant cells from Excel into vim / other text editors directly. See cp3_calc.txt and cp3_expt.txt in this folder for
# a real-life example.
#
# If the experimental shifts are the same (i.e. Delta_e = 0), the entire row will effectively be excluded from the
# calculation. There is no explicit code for that, but x / 0 evaluates to NaN which behaves as zero in a sum, so it
# makes no contribution to the CP3 value.
#
# The output follows the Goodman paper quite closely and should be fairly self-explanatory. A and B are the two possible
# assignments for each pair of isomers. CP3(A) and CP3(B) are the values of the CP3 parameter, evaluated for both
# possible assignments. P(CP3|A) is the probability of observing both CP3(A) and CP3(B) given that A is the correct
# assignment (this is simply the product of P(CP3(A)|A) and P(CP3(B)|A)). Then, P(A|CP3) is the probability of A being
# the correct assignment given the CP3 values calculated for both assignments. This last number is the "confidence" that
# we have in predicting a certain assignment, i.e. if P(A|CP3) is 0.95, then we can say that CP3 allows us to predict
# that assignment A is correct with 95% certainty.
#
...
# pairs of columns; so, for example, if there are three columns, it will provide CP3 parameters for assigning the first
# two columns of cp3_expt.txt to the first two columns of cp3_calc.txt, then it will do the same for the first and third
# columns of both files, then again for the last two columns of both files.
# This is only really useful if you already know which experimental shifts belong to which isomer and simply want to
# check whether the calculated data allows for correct prediction of all pairs of diastereomers. DO NOT use this to try
# to assign three sets of experimental data to three sets of computational data! The CP3 parameter is NOT designed for
# this use! Instead, consider using the DP4 method instead.
#
...
if __name__ == '__main__':
    ...
    calc_df = parse_data("cp3_calc.txt")
    expt_df = parse_data("cp3_expt.txt")
    ...
    if len(calc_df.columns) != len(expt_df.columns):
        error_quit("Number of isomers in cp3_calc.txt and cp3_expt.txt differ.")
    for i in range(len(calc_df)):
        if not calc_df['label'][i] == expt_df['label'][i]:
            error_quit("Atom labels {} in cp3_calc.txt and {} in cp3_expt.txt do not match.".format(calc_df['label'][i],
                                                                                                    expt_df['label'][i]))
        if calc_df['atom'][i] not in ["C", "c", "H", "h"]:
            error_quit("Atom label {} in cp3_calc.txt does not begin with C/c/H/H.".format(calc_df['label'][i]))
        if expt_df['atom'][i] not in ["C", "c", "H", "h"]:
            error_quit("Atom label {} in cp3_expt.txt does not begin with C/c/H/H.".format(expt_df['label'][i]))
